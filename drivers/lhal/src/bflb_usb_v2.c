...
int usbd_ep_open(uint8_t busid, const struct usb_endpoint_descriptor *ep_cfg)
{
    ...
    ep = ep_cfg->bEndpointAddress;
    ep_mps = USB_GET_MAXPACKETSIZE(ep_cfg->wMaxPacketSize);
    ep_attr = USB_GET_ENDPOINT_TYPE(ep_cfg->bmAttributes);
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        g_bl_udc.out_ep[ep_idx].ep_mps = ep_mps;
        g_bl_udc.out_ep[ep_idx].ep_type = ep_attr;
        g_bl_udc.out_ep[ep_idx].ep_enable = 1U;
        bflb_usb_set_outep_mps(ep_idx, ep_mps);
    } else {
        g_bl_udc.in_ep[ep_idx].ep_mps = ep_mps;
        g_bl_udc.in_ep[ep_idx].ep_type = ep_attr;
        g_bl_udc.in_ep[ep_idx].ep_enable = 1U;
        bflb_usb_set_inep_mps(ep_idx, ep_mps);
    ...
    if (ep_idx != 0) {
        ...
        if (ep_mps > 512) {
            ...
            if (ep_idx == 1) {
                bflb_usb_fifo_config(USB_FIFO_F0, ep_attr, 1024, 1, true);
                bflb_usb_fifo_config(USB_FIFO_F1, ep_attr, 1024, 1, false);
            } else if (ep_idx == 2) {
                bflb_usb_fifo_config(USB_FIFO_F2, ep_attr, 1024, 1, true);
                bflb_usb_fifo_config(USB_FIFO_F3, ep_attr, 1024, 1, false);
            } else {
                return -1;
            }
        } else {
            ...
            if (ep_idx == 1) {
                bflb_usb_fifo_config(USB_FIFO_F0, ep_attr, 512, 1, true);
            } else if (ep_idx == 2) {
                bflb_usb_fifo_config(USB_FIFO_F1, ep_attr, 512, 1, true);
            } else if (ep_idx == 3) {
                bflb_usb_fifo_config(USB_FIFO_F2, ep_attr, 512, 1, true);
            } else if (ep_idx == 4) {
                bflb_usb_fifo_config(USB_FIFO_F3, ep_attr, 512, 1, true);
            } else {
                return -1;
            }
        }
        ...
        if (ep_mps > 512) {
            ...
            if (ep_idx == 1) {
                bflb_usb_fifo_config(USB_FIFO_F0, ep_attr, 1024, 2, true);
                bflb_usb_fifo_config(USB_FIFO_F1, ep_attr, 1024, 2, false);
                bflb_usb_fifo_config(USB_FIFO_F2, ep_attr, 1024, 2, false);
                bflb_usb_fifo_config(USB_FIFO_F3, ep_attr, 1024, 2, false);
            } else {
                return -1;
            }
        } else {
            ...
            if (ep_idx == 1) {
                bflb_usb_fifo_config(USB_FIFO_F0, ep_attr, 512, 2, true);
                bflb_usb_fifo_config(USB_FIFO_F1, ep_attr, 512, 2, false);
            } else if (ep_idx == 2) {
                bflb_usb_fifo_config(USB_FIFO_F2, ep_attr, 512, 2, true);
                bflb_usb_fifo_config(USB_FIFO_F3, ep_attr, 512, 2, false);
            } else {
                return -1;
            }
        }
